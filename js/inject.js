var options, setOptions, radData, kanData, prevItem;
var kunyomi_list = [
...
function powerLevel(powerSetting) {
	var powerSetting = options.power_level;
	return powerSetting && powerSetting > 0;
}
...
function setReviewQueue(needsUpdating) {
	var currRad = !options.sort_rad ? localStorage.getItem('radicalData') : null;
	var storedReviews = $.jStorage.get("reviewQueue");
	if (!needsUpdating) {
		for (var i = 0; i < storedReviews.length; ++i) {
			var item = storedReviews[i];
			if (item && levelIndex(item, currRad, null) !== 2) {
				needsUpdating = true;
				break;
			}
		}
	...
	for (var i = 0; i < storedReviews.length; ++i) {
		var item = storedReviews[i];
		if (item) {
			var li = levelIndex(item, currRad, null);
	...
	$('#current-count').text(currCount);
	$('#customizer-stats').toggle(currCount > 0);
}
...
if ($.jStorage && $('#lessons').length == 0) {
	...
	$.jStorage.listenKeyChange('currentItem', function(key, action) {
		...
		$('.icon-chevron-right').toggleClass('icon-burn', burning);
...
$(function() {
	...
	if ($('#lessons').length > 0) {
		...
		var itemCount = $('li.reviews a span').text();
		...
		itemCount = $('li.lessons a span').text();
		if (itemCount) {
			if (itemCount != '0') {
				var loadLessons = powerLevel();
				if (!loadLessons) {
		...
		if (powerLevel()) {
			var nextTimeDisplay = $('li.next time');
			if (nextTimeDisplay) {
